...
static OVERLAPPED	 stcWriteStatus[TOTAL_PORT_NUM] = { 0 }; //OVERLAPPED: 使程序可异步操作(可以在后台运行，让前台执行其他程序)
...
signed char OpenCOMDevice(const unsigned long ulPortNo, const unsigned long ulBaundrate)
{
	...
	if ((hComDev[ulPortNo] = CreateFile(PortName, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 
		FILE_FLAG_OVERLAPPED, NULL)) == INVALID_HANDLE_VALUE)
	{
		dwRes=GetLastError();
//		printf("\nError: Fail to create a new COM.\n");
		return -1;
	}
